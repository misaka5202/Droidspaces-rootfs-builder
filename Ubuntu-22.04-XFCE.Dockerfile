...
RUN apt-get update && \
    ...
    # 最小化 XFCE
    if [ "$BUILD_XFCE" = "min" ]; then \
        apt-get install -y --no-install-recommends \
        dbus-x11 x11-xserver-utils xinit xorg fonts-noto-cjk fonts-noto-color-emoji \
        xfce4 xfce4-terminal xfce4-session xfwm4 xfconf thunar \
        pulseaudio pulseaudio-utils mesa-utils vulkan-tools \
        dbus-user-session policykit-1 at-spi2-core tumbler xdg-user-dirs; \
    fi && \
    # 精简 XFCE（较完整桌面组件）
    if [ "$BUILD_XFCE" = "conc" ]; then \
        apt-get install -y --no-install-recommends \
        dbus-x11 x11-xserver-utils x11-utils xinit xorg xclip xsel \
        fonts-noto-cjk fonts-noto-color-emoji fonts-dejavu-core fonts-liberation fonts-liberation2 fonts-noto-core fonts-noto-ui-core fonts-ubuntu \
        xfce4 desktop-base xfce4-terminal xfce4-session xscreensaver xfce4-goodies \
        xubuntu-wallpapers xfce4-taskmanager mousepad galculator ristretto \
        xfce4-screenshooter catfish mugshot xcursor-themes dmz-cursor-theme xfce4-clipman-plugin \
        thunar thunar-volman thunar-archive-plugin thunar-media-tags-plugin \
        gvfs gvfs-backends gvfs-fuse xfwm4 xfconf zenity notification-daemon \
        adwaita-icon-theme-full hicolor-icon-theme gnome-icon-theme tango-icon-theme \
        gtk2-engines-murrine gtk2-engines-pixbuf arc-theme numix-gtk-theme \
        materia-gtk-theme papirus-icon-theme greybird-gtk-theme \
        pulseaudio pulseaudio-utils pavucontrol \
        mesa-utils vulkan-tools dbus-user-session policykit-1 at-spi2-core tumbler xdg-user-dirs \
...
# 输入法开机自启动 / Mesa 环境 / XFCE 自启动服务
COPY scripts/xfce-start /usr/local/bin/xfce-start
RUN <<'EOF_RUN'
    chmod +x /usr/local/bin/xfce-start
    if [ "$ENABLE_srf_ARG" = "true" ]; then
...
EOF
    ...
    if [ "$BUILD_XFCE_plus" = "true" ] ; then
        cat > /etc/systemd/system/xfce-autostart.service << 'EOF'
[Unit]
...
EOF
        chmod 644 /etc/systemd/system/xfce-autostart.service
...
# Snapdragon GPU：mesa-for-android-container（优先 jammy，无则回退 noble）
RUN if [ "$ENABLE_mesa_ARG" = "true" ]; then \
        echo "--> [开启] 正在下载并安装最新版 Mesa 驱动..." && \
        RELEASES_JSON=$(curl -s https://api.github.com/repos/lfdevs/mesa-for-android-container/releases/latest) && \
        URL=$(echo "$RELEASES_JSON" | jq -r '.assets[] | select(.name | test("mesa-for-android-container_.*_ubuntu_jammy_arm64\\.tar\\.gz")) | .browser_download_url' | head -1) && \
        if [ -z "$URL" ] || [ "$URL" = "null" ]; then \
            echo "--> [提示] 未找到 jammy 包，回退到 ubuntu_noble_arm64..." && \
            URL=$(echo "$RELEASES_JSON" | jq -r '.assets[] | select(.name | test("mesa-for-android-container_.*_ubuntu_noble_arm64\\.tar\\.gz")) | .browser_download_url' | head -1); \
        fi && \
...
for unit in systemd-udevd.service systemd-udev-trigger.service systemd-udev-settle.service systemd-udevd-kernel.socket systemd-udevd-control.socket; do
    ...
    printf "[Unit]\nConditionPathIsReadWrite=\n" > "/etc/systemd/system/${unit}.d/99-readonly-fs.conf"
done
...
# XFCE Turnip 合成器修复（与 Ubuntu-24.04-XFCE 相同，若仓库提供 xfwm4.xml）
...
RUN if [ "$BUILD_XFCE" = "min" ] || [ "$BUILD_XFCE" = "conc" ]; then \
...
# Ubuntu 22.04 使用传统 sources.list（非 deb822 ubuntu.sources）
